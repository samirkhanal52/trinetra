...
from src.utils import get_s3_key_for_screenshot, safe_create_dir
...
class UploaderService:
    ...
    def __init__(self, max_retries: int = 3):
        ...
        safe_create_dir(config.OUTBOX_DIR)
    ...
    def _process_and_upload(self, image_path, metadata_path):
        ...
        try:
            ...
            s3_key = get_s3_key_for_screenshot(config.S3_PREFIX, timestamp, image_uuid)
            ...
            if success:
                ...
                logging.info(
                    f"Cleaned up local files for {os.path.basename(image_path)}"
                )

        except (FileNotFoundError, json.JSONDecodeError, IndexError) as e:
            logging.warning(
                f"Skipping corrupted/incomplete file pair: {image_path}. Error: {e}"
            )
        except Exception as e:
        ...
        for attempt in range(self.max_retries):
            try:
                self.s3_client.upload_file(
                    local_path, config.S3_BUCKET_NAME, s3_key, ExtraArgs=extra_args
                )
                logging.info(
                    f"Successfully uploaded {local_path} to s3://{config.S3_BUCKET_NAME}/{s3_key}"
                )
                return True
            except (ClientError, NoCredentialsError) as e:
                logging.error(
                    f"S3 upload failed for {s3_key} (attempt {attempt + 1}): {e}"
                )
    ...
    def _ensure_bucket_exists(self):
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                logging.info(
                    f"Bucket '{config.S3_BUCKET_NAME}' not found. Creating it..."
                )
                try:
                    # For AWS, specifying a region other than us-east-1 requires LocationConstraint
                    if (
                        config.AWS_REGION
                        and config.AWS_REGION != "us-east-1"
                        and config.TRINETRA_ENV == "prod"
                    ):
                    ...
                    logging.info(
                        f"Bucket '{config.S3_BUCKET_NAME}' created successfully."
                    )
